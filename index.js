...
(function( factory ) {
    ...
    function extract( text ) {
        ...
        function extractor() {
            return _capturesToJson(self.captures, self.exec( text ));   // jshint ignore: line
        }

        if ( self.global ) {                                            // jshint ignore: line
            self.lastIndex = 0;
            all = [];
            while ( (json = extractor()) ) {                            // jshint ignore: line
                all.push( json );
            }
            return all;
        }
     ...
     *  'F'
     * );
     * regex.replace("the temp is 11.3F", "${temp}");
    ...
    function replace( text, replacement ) {
        ...
        function replacer() {
            ...
            return replacement(json, offset, text);
        }
        ...
        if ( typeof replacement === 'string' ) {
            ...
            if ( ! replacementCache[ replacement ] ) {
                replacementCache[ replacement ] = replacement.replace(varRegex, function(matches, variable) {
                    var idx = self.captures.indexOf(variable);
